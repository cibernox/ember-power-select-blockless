...
test('Renders the property in the given path on each option and the trigger when options are objects', function(assert) {
  ...
  this.users = users;
  this.user = [users[1]];
  this.render(hbs`{{power-select-multiple-blockless options=users labelPath="name" selected=user onchange=(action (mut user))}}`);
...
test('Renders each string in the list without needing any labelPath and the trigger when options are strings', function(assert) {
  ...
  this.names = names;
  this.name = [names[1]];
  this.render(hbs`{{power-select-multiple-blockless options=names selected=name onchange=(action (mut name))}}`);
  assert.equal(this.$().text().replace("×", "").trim(), 'Joe Hardy');
  clickTrigger();
