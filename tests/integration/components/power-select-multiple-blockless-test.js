...
module('Integration | Component | power select blockless', function(hooks) {
  ...
  test('Renders the property in the given path on each option and the trigger when options are objects', async function(assert) {
    ...
    this.users = users;
    this.user = [users[1]];
    await render(hbs`{{power-select-multiple-blockless options=users labelPath="name" selected=user onchange=(action (mut user))}}`);
    assert.equal(this.$().text().replace("×", "").trim(), 'Joe Hardy');
    await clickTrigger();
  ...
  test('Renders each string in the list without needing any labelPath and the trigger when options are strings', async function(assert) {
    ...
    this.names = names;
    this.name = [names[1]];
    await render(hbs`{{power-select-multiple-blockless options=names selected=name onchange=(action (mut name))}}`);
    assert.equal(this.$().text().replace("×", "").trim(), 'Joe Hardy');
    await clickTrigger();
